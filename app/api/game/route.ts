...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: false, error: "Server error while loading game state" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: false, error: "Server error while handling game action" }, { status: 500 });
  }
}
